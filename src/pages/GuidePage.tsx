    ...
    <main className="flex flex-1 flex-col px-4 pb-8 pt-6">
      ...
      <article className="space-y-8">
        <Section id="about" title="cast mark とは">
          <p>
            {SERVICE_NAME}
            は、釣り場で「いま」の条件と釣果をワンタップで残す、自分のための記録アプリです。スマホのブラウザで開き、ホーム画面に追加して使えます。ノートや地図アプリを行き来せず、釣れた瞬間の位置・天気・潮位を同じ記録にまとめ、自分だけの釣果マップを作れます。
          </p>
          <p>
            投稿やフォロー、他の人のポイント共有はありません。公開の釣り場データベースでもありません。残した座標や写真はアカウントに紐づいて保存され、本人以外は見られません。気負わず、自分のペースで始められます。
          </p>
        </Section>
        ...
        <Section id="features" title="できること">
          ...
          <ul className="list-disc space-y-2 pl-5">
            <li>
              ...
              「記録する」を押すと、現在地、気温・天気、潮位を取得して保存します。魚種、匹数、体長、重さ、写真、タックルは任意です。先に「釣行を開始」すると、その場の条件を取っておき、あとの記録では使い回します。何も釣らずに「釣行を終了」すると、ボウズとして残ります。
            </li>
            <li>
              <strong className="font-medium text-sky-950">履歴:</strong>{' '}
              日付カレンダー、地図、一覧で過去の釣果を振り返れます。魚種・場所・ルアー／エサ・ボウズで絞り込めます。同じ釣行の記録はまとめて表示されます。記録を開いて編集・削除できます。
            </li>
            <li>
              <strong className="font-medium text-sky-950">マイ魚種図鑑:</strong>{' '}
              釣った魚種数と釣果数の合計、魚種ごとの尾数や最大サイズを、記録から自動で集計します。魚種名や別名で検索できます。魚種を開くと、釣れたときの時間帯・潮・場所・ルアーの内訳も出ます。
            </li>
            <li>
              <strong className="font-medium text-sky-950">マイタックル:</strong>{' '}
              ロッド、リール、ライン、ルアーまたはエサ、仕掛けをセットとして保存し、記録時に呼び出せます。
            </li>
            <li>
        ...
        <Section id="record" title="記録のしかた">
          ...
          <ol className="list-decimal space-y-2 pl-5">
            ...
            <li>同じ種類をまとめて残すときは匹数を入れます。空なら図鑑では 1 匹として数えます。</li>
            <li>体長と重さを入れます。単位はマイページで cm / inch、g / kg / oz を切り替えられます。保存値は変わりません。</li>
            <li>
              必要なら「タックル入力を開く」から、セット名、ロッド、リール、ライン、ルアーまたはエサ、仕掛けを記入します。魚種と同じく、よく使う種類や有名な機種名の候補から選べます。ルアーとエサは切替でラベルと候補が変わります。よく使う組み合わせはマイタックルに保存し、次回呼び出してください。「次回もこのタックルを使う」をオンにすると、記録後も入力が残ります。
            </li>
            <li>
          ...
          <p>
            保存が終わると、その場のサマリーが表示されます。「続けて記録」で同じ釣行の次の一尾へ進めます。位置・天気・潮位は最初のものを使います。着いた時点で「釣行を開始」しておくと、条件だけ先に取れます。終えるときは「釣行を終了」です。その間に記録がなければボウズとして残ります。
          </p>
        </Section>

        <Section id="history" title="履歴と地図">
          <p>
            「履歴」タブでは、保存した記録を日付と場所で振り返ります。カレンダーで開始日と終了日をタップすると期間を絞り込めます。さらに魚種・場所・ルアー／エサ・ボウズでも絞り込めます。期間の下には地図があり、記録した地点が並びます。地図のピンをタップすると、その場所の記録が開きます。同じ釣行で残した複数の記録は、一覧でまとめて表示されます。
          </p>
          <HistoryScreenMock />
          <p>
        ...
        <Section id="mypage" title="マイページ">
          <p>
            画面下の「図鑑」タブがマイ魚種図鑑です。これまでに残した記録を魚種ごとに集計します。先頭に魚種数と釣果数の合計が出ます。魚種名や別名で検索できます。尾数や最大サイズで並べ替えられます。魚種を選ぶと、その魚が釣れたときの時間帯・潮・場所・ルアーの内訳と、時系列の記録を見られます。タックル帳、単位、アカウント設定は、その右の「マイ」タブから開きます。
          </p>
          <EncyclopediaScreenMock />
          <p>
        ...
        <Section id="faq" title="よくある質問">
          ...
          <p>
            <strong className="font-medium text-sky-950">SNSで釣果を公開しますか。</strong>
            しません。投稿・フォロー・タイムラインはありません。記録はログインした本人だけが見られます。
          </p>
          <p>
      ...
      <div className="mt-8 rounded-xl border border-sky-200 bg-white px-4 py-4 shadow-sm">
        ...
        <p className="mt-1 text-sm text-slate-500">
          アカウントを作ると、記録の同期と履歴の振り返りが使えます。投稿は不要です。
        </p>
        <Link
          to={startHref}
