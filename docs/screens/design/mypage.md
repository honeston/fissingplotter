...
同期バナー（ログイン中と同じ行）から開く。単位は表示と入力だけ変わり、保存済みの数値は変わらない。アカウント欄はクラウド有効時のみ。ローカルのみのときは「この端末に記録を保存しています」と出す。
...
記録から魚種ごとの尾数・最大サイズなどを集計する。一覧の先頭に魚種数と釣果数の合計を出す（魚種なしは含めない）。一覧カードにはその魚種の代表画像（写真付き記録のうち最大サイズ）を出す。手入力の図鑑ではない。
