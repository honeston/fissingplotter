...
現行コード（`api/src/handler.ts`、`infra/template.yaml`、クライアント `src/lib/api.ts`）に基づく HTTP API 定義。全体像は [API 概要](overview.md)。パスと処理の流れは [API 設計](design/README.md)。画面側の項目は [画面 IF](../screens/if.md)。
