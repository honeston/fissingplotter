...
| 単位 | `src/lib/units.ts` | cm/inch、g/kg/oz の変換。保存値は cm / g のまま | [UNIT-04](../if.md#unit-04-単位変換) |
...
| 日付 | `src/lib/dates.ts` | 履歴の日キー、期間 | [UNIT-08](../if.md#unit-08-日付) |
...
| 記録バリデーション | `api/src/records.ts`（または同等） | 必須欠落、負の体長、緯度だけ | [UNIT-12](../if.md#unit-12-記録バリデーション) |
...
- `sync.ts` のネットワーク分岐（E2E。マージ規則の純関数化ができたら UNIT に下ろす）
