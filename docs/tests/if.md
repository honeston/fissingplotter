...
| ID | 優先 | 入力 | 期待 |
|----|------|------|------|
| UNIT-05a | P1 | アジ 2 件（サイズ違い）、魚種なし 1 件 | アジのみ。匹数 2。最大サイズは大きい方。魚種なしは除外 |
| UNIT-05b | P1 | 写真あり／なしが混在。端末 Blob ID と `photoKey` | 代表は写真付きのうち最大サイズ。同サイズなら新しい方。写真なしだけなら null |
| UNIT-05c | P1 | アジ 2 件、メバル 1 件、魚種なし 1 件 | 魚種合計 2、釣果合計 3。魚種なしは含めない |
| UNIT-05d | P1 | アジ・スズキ・メバル。クエリ空 / あじ / シーバス / いない魚 | 空は 3 件。あじはアジ。シーバスはスズキ。いない魚は 0。合計は絞る前のまま |
| UNIT-05e | P1 | アジ 1 件 `fishCount` 3、別日のアジ 1 件（未入力） | 匹数 4。最大釣果日は 3 匹の日 |

対象: `buildSpeciesStats` / `pickCoverRecord` / `encyclopediaTotals` / `filterSpeciesStats`。[SCR-12](../screens/if.md#scr-12-マイ魚種図鑑)
...
| UNIT-12h | P1 | fishCount 0 / 小数 / 負 | `Invalid fishCount` |
...
対象: `src/lib/speciesCatchStats.ts`。[SCR-13](../screens/if.md#scr-13-魚種詳細)

| ID | 優先 | 入力 | 期待 |
|----|------|------|------|
| UNIT-15a | P1 | 日出前後・日中・日没前後・夜間。日出没あり | 朝マヅメ / 日中 / 夕マヅメ / 夜。欠ける記録は時間帯の母数外 |
| UNIT-15b | P1 | 潮の傾き +15 / −8 / 0.4 / なし | 上げ / 下げ / 横ばい。なしは母数外 |
| UNIT-15c | P1 | 同一地点付近 2 件と 1km 先 1 件。場所名あり／なし | 近い 2 件が同一地点。表示は最多の場所名。離れた件は別 |
| UNIT-15d | P1 | ルアー「ミノー」2 件（一方は `fishCount` 3）と空 1 件 | ミノー 4 匹。日数はミノーの日だけ。空は母数外 |
| UNIT-15e | P1 | アジ 4 匹のうち朝マヅメ 3。他軸はばらける | 要約に朝マヅメ。3 匹未満は要約なし。最頻が同数ならその軸は要約に使わない |
| UNIT-15f | P1 | クエリ `slot=dawn` / 不明値 / 複数キー | 朝マヅメの記録だけ。不明は全件。複数は slot を優先 |

### UNIT-16 タックル候補

対象: `src/lib/tackleSuggestions.ts`。[COM-05](../screens/if.md#com-05-タックル入力-tacklefieldsform)

辞書の網羅はしない。代表ケースだけ。

| ID | 優先 | 入力 | 期待 |
|----|------|------|------|
| UNIT-16a | P1 | ルアー欄・空クエリ | 先頭は種類名（ミノー） |
| UNIT-16b | P1 | ひらがな「みのー」 | ミノーが含まれる |
| UNIT-16c | P1 | エサ切替・空クエリ | アオイソメが含まれる。ミノーは含まれない |
| UNIT-16d | P1 | ロッド・履歴ありの空クエリ | 履歴がカタログより先 |
| UNIT-16e | P1 | 別名 minnow / stella / PE1.0。未知の竿名 | ミノー / ステラ / PE 1.0号。未知はそのまま |
| UNIT-16f | P1 | 記録（新しい竿＋ルアー、エサ＋サビキ）とマイタックル | ロッドは新しい順。ルアー／エサ／仕掛けは kind で分かれる |
...
| E2E-03b | 無 | P1 | 魚種・体長を入れて保存 | サマリーに反映。続けて記録で魚種・サイズ・匹数はクリア |
| E2E-03c | 無 | P1 | 体長に -1 | 「体長は 0 以上…」。未保存 |
| E2E-03d | 無 | P2 | geolocation 失敗 | それでも保存される |
| E2E-03e | 無 | P1 | 魚種・匹数 3 で保存 | サマリーに 3匹。続けて記録で匹数はクリア |
| E2E-03f | 無 | P1 | 匹数に 0 | 「匹数は 1 以上の整数で入力してください」。未保存 |
...
| E2E-04d | P2 | タックルを開きルアー候補から選ぶ | ルアー欄に選んだ値が入る |
...
| ID | 優先 | 操作 | 期待 |
|----|------|------|------|
| E2E-07a | P2 | 魚種なし記録だけ | 合計 0種 / 0匹。「魚種付きの記録がまだありません」 |
| E2E-07b | P2 | アジを保存（写真なし） → 図鑑 | 合計 1種 / 1匹。アジカード。代表画像なし。詳細でその記録 |
| E2E-07c | P2 | 写真付きアジを保存 → 図鑑 | カードに「アジの代表画像」 |
| E2E-07d | P2 | アジとヒラメを保存 → 図鑑で「アジ」検索 → いない魚 → クリア | アジだけ。合計は 2種のまま。ヒットなし文。クリアで 2 種 |
| E2E-07e | P2 | アジを匹数 3 で保存 → 図鑑 | 合計 1種 / 3匹 |
| E2E-07f | P2 | アジ + ルアー「ミノー」で保存 → 詳細 | 条件は初期閉じ。開くとミノー。タップで絞り込み。解除で全件 |

モード: 無。[SCR-12](../screens/if.md#scr-12-マイ魚種図鑑) / [SCR-13](../screens/if.md#scr-13-魚種詳細)
...
| ID | 優先 | 操作 | 期待 |
|----|------|------|------|
| E2E-11a | P2 | ログイン相当で `/` | COM-01（ログイン中）と COM-02（記録 / 履歴 / マイ図鑑 / マイページ）。記録タブアクティブ |
...
| 操作 | 期待 |
|------|------|
| キーありで記録 | 気温・場所名・潮位が入ることがある。詳細シートに当日の潮位グラフ（満干・記録時刻の印） |
